...
export abstract class StoreableList<T> extends SelectableList<T> implements IStoreableList<T> {
  /**
  ...
  save(): void {
    this.sort();
    try {
      localStorage.setItem(this.LOCAL_STORAGE_KEY, JSON.stringify(this.list));
    } catch (error) {
      // put something here
    }
  }
